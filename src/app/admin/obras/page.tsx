...
export default async function AdminObras() {
  ...
  const { data: works } = await supabase
    .from('works')
    .select('*, authors(name), chapters(lang)')
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: '#111009' }}>
              {['Título', 'Autor', 'Tipo', 'Idiomas', 'Capítulos', ''].map((h) => (
          ...
          <tbody>
            {(works ?? []).map((w: any, i: number) => {
              const langs = ([...new Set((w.chapters ?? []).map((c: any) => c.lang ?? 'es'))] as string[]).sort();
              const chapCount = (w.chapters ?? []).length;
              return (
                <tr key={w.id} style={{ background: i % 2 === 0 ? '#1A1816' : '#161411', borderTop: '1px solid #2A2720' }}>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-1">
                      {langs.length === 0
                        ? <span className="text-xs" style={{ color: '#8A8478' }}>—</span>
                        : langs.map((lang: string) => (
                          <span key={lang} className="text-xs px-2 py-0.5 rounded-full font-semibold uppercase"
                  ...
                  <td className="px-5 py-3.5 text-sm" style={{ color: '#8A8478' }}>
                    {chapCount > 0 ? chapCount : <span style={{ color: '#4A4438' }}>sin audio</span>}
                  </td>
