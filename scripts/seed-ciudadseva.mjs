/**
 * Seed de cuentos ciudadseva -- genera audio ES para los 56 cuentos.
 *
 * Sin API keys externas. Solo Node.js + msedge-tts + Supabase.
 *
 * Uso:
 *   node scripts/seed-ciudadseva.mjs              # todos los pendientes
 *   node scripts/seed-ciudadseva.mjs jack-london  # solo ese autor
 *   node scripts/seed-ciudadseva.mjs el-collar    # solo ese cuento
 *
 * Resumible: salta los cuentos que ya tienen audio.
 */
...
// -- Filtro por argumento CLI -----------------------------------------------

const filterArg = process.argv[2]?.toLowerCase();
let storiesToProcess = STORIES.filter(s => !s.skipIfExists);
if (filterArg) {
  storiesToProcess = storiesToProcess.filter(
    s => s.slug === filterArg || s.authorSlug === filterArg || s.authorSlug.includes(filterArg)
  );
  if (storiesToProcess.length === 0) {
    console.error('No se encontraron cuentos con filtro: ' + filterArg);
    process.exit(1);
  }
  console.log('Filtro: "' + filterArg + '" -> ' + storiesToProcess.length + ' cuento(s)\n');
}
...
// -- Decodificacion de HTML ------------------------------------------------
// Usamos unicode escapes para caracteres especiales y evitar problemas de
// encoding en el archivo fuente.
...
async function scrapeCiudadseva(slug) {
  ...
  if (!divMatch) throw new Error('No se encontro el texto en la pagina');
  ...
  if (pMatches.length > 0) {
    ...
    let text = divMatch[1].replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '');
...
// -- Generacion de audio con timings ---------------------------------------
...
async function uploadAndSave(workSlug, workId, voice, title, text) {
  ...
  console.log('OK ' + mb + 'MB - ' + timings.length + ' parrafos');
}

// -- Verificar si ya existe ------------------------------------------------

async function chapterExists(workId) {
  const { data } = await supabase.from('chapters')
    .select('id').eq('work_id', workId).eq('chapter_number', 1).eq('lang', 'es').maybeSingle();
  return !!data;
}
...
console.log('-------------------------------------------------------');
console.log('Seed ciudadseva -- ' + storiesToProcess.length + ' cuentos (solo ES)');
console.log('-------------------------------------------------------');
...
for (const story of storiesToProcess) {
  ...
  if (await chapterExists(workId)) {
    console.log('Saltando (ya tiene audio): ' + story.slug);
    skipped++;
    continue;
  }

  process.stdout.write('Procesando: ' + story.slug + ' ... ');
  ...
  try {
    ...
    failed++;
    errors.push({ slug: story.slug, error: e.message });
  }
...
console.log('\n-------------------------------------------------------');
console.log('Procesados: ' + processed + '  Salteados: ' + skipped + '  Errores: ' + failed);
if (errors.length > 0) {
  console.log('\nErrores:');
